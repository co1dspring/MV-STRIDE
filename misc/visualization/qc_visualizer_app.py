...
# Custom CSS: scrolling containers, larger text, and layout
st.markdown("""
    <style>
    /* 强制图片水平排列的滚动容器 */
    .scroll-container {
...
def main():
    ...
    st.markdown("#### 🖼️ 图像序列")
    if img_list:
        # Use columns to simulate a horizontal layout
    ...
    with col_qc:
        ...
        c1, c2 = st.columns(2)
        # Auto-advance to the next item after clicking
    ...
    if st.sidebar.button("💾 导出最终质检 JSON", use_container_width=True):
        ...
        for i, item in enumerate(data):
            res_item = item.copy()
            # Get the QC result; mark as unreviewed if not yet checked
