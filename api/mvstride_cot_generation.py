...
class APIDataProcesser:
    def __init__(self, config):
        ...
        **Task:**
        ...
        Your objective is to generate a long, coherent Chain of Thought (CoT) specifically for the final Level 3 question according to the information of input images and the logic and conclusions of level 1 and 2 QAs. 
        
        **Reasoning Guidelines:**
        When generating the reasoning process, you must follow the sequence below to construct a chain of thought that aligns with the human spatial cognition process. The reasoning is divided into three major stages, and each stage can be further broken down into specific logical steps as needed.
        Stage 1: Single-View Perception: Identify the 2D and 3D spatial information of cameras and objects in images as the basis for reasoning.
        Stage 2: Cross-View 3D Modeling: Integrate information from different perspectives to determine camera transformations and object correspondences. Construct a 3D cognitive map to memory the relative positions and orientations of all elements in the global scene.
        Stage 3: High-Level Contextual Inference: Integrate global information to perform high-order spatial reasoning in a 3D consistent manner, eg virtual viewpoint imagination.
        
        **Strict Requirements:**
        1. Simulated Discovery & anchor: Please assume that you have only received the image and the Level 3 question, and try to generate a multi-stage reasoning process and answer for the question. You can directly follow the thought progression and answers from Level 1 and 2, and pretend that you have discovered or derived these conclusions on your own, rather than directly quoting them. You need to naturally transform the Q&A content of level x into the reasoning process of stage x, and always ensure that the reasoning results and the question answers remain consistent, forming a logical and coherent cycle.
        2. Image-Text Verification: For most of the time, you can trust the authenticity of all questions and answers, especially the level 3 answer is definitely correct. But images are the "source of truth." If you find there are inconsistencies, omissions, or errors between the QA content and the image content, you must prioritize correcting your reasoning path based on the visual evidence in the image. You can also add some facts you have observed from the images to make the reasoning logic more coherent.
        3. Logical Connectivity: Explicitly bridge the gap between different levels and different question types in the same level. Explain how the perception in Level 1 supports the modeling in Level 2, and how that modeling enables the final inference in Level 3.
        
        **Output Format:**
        You must generate a comprehensive reasoning process, and place the reasoning process within <think></think> tags and place the final answer within <answer></answer> tags.
        Even if the answer seems obvious from the provided facts, you are **STRICTLY PROHIBITED** from skipping the <think></think> stage. A response without a detailed <think></think> section is considered a failure.
        Here is an example:
        <think>
        Stage 1 – Single-view perception
        - In Figure 1, I observe one prominent tabletop across the lower-left foreground (bounding box [0, 793, 1143, 1439]).
        - In Figure 2, I observe two separate tabletops: one stretching across the right side (bounding box [1122, 162, 1919, 1180]) and a shallow band-like tabletop along the upper center (bounding box [707, 0, 1226, 218]).
        Stage 2 – Cross-view 3D modeling
        - The viewpoints differ: Figure 1 looks from near the front-right toward the room, while Figure 2 is taken more from the left/side, looking along rows of desks.
        - Comparing shapes, extents, and placements relative to monitors and room boundaries, none of the Figure 2 tabletops align in position/appearance with the single large foreground tabletop in Figure 1. Each appears to be a different physical desk segment, likely in other rows or out of Figure 1’s field of view.
        Stage 3 – High-Level Contextual Reasoning
        - Since the two tabletops in Figure 2 do not correspond to the one in Figure 1, all three are distinct physical tables.
        - Total unique tables across both images = 2 (Figure 2) + 1 (Figure 1) = 3.
        [The reasoning process here is generated according to Stage 1-3 of Reasoning Guidelines. You can split steps in every stage.]
        </think>
        <answer>B: 3[Final Answer]</answer>
        """
    ...
    def _format_user_prompt(self, item):
        """将多轮 Level 1-3 转化为推理型 Prompt"""
        # 如果已经手动传入了 prompt 则使用，否则构建
        user_prompt = ""
        for i in range(int(len(item['messages'])/2)):
            user_prompt += item['messages'][2 * i]['content']
            user_prompt += f'\n{item['messages'][2 * i + 1]['content']}\n'
        user_prompt = user_prompt.replace("<video>", "").replace("<image>", "")
...
if __name__ == "__main__":
    ...
    my_config = {
        'input_file_path': 'D:/Data/infinigen_20251031/QA_jsons_ScannetppIphone_MultilevelCategories_20260124_sampled_MCA_Multistage/ScannetppIphone_MultilevelCategories_20260124_sampled_MCA_Multistage_stage2.json',
