...
class APIDataProcesser:
    ...
    def _format_user_prompt(self, item):
        """
        拼装成用于校验的 User Prompt：
        包含：核心问题、GT 黄金事实多轮对话、待校验的 Candidate CoT & Answer。
        """
        # 1. 获取核心问题并清洗
        ...
        # 使用正则从生成的 content 中分离 think 和 answer（防止格式不规整）
        candidate_think = "No think block found."
        candidate_answer = "No answer block found."

        think_match = re.search(r"<think>(.*?)</think>", candidate_assistant_content, re.DOTALL | re.IGNORECASE)
        if think_match:
        ...
        if ans_match:
            ...
            # 如果没包在 answer 标签里，尝试取 think 后面的所有文本
            candidate_answer = re.sub(r"<think>.*?</think>", "", candidate_assistant_content, flags=re.DOTALL | re.IGNORECASE).strip()
        ...
        # user_prompt += "Please check if the candidate's perception (Stage 1), cross-view modeling (Stage 2), and rotation/translation logic (Stage 3) perfectly align with the Ground Truth observations. Then output your Verdict, Error Analysis, and Corrected Answer."

        user_prompt += (
            "Please compare the Candidate CoT with the GT Observations and the GT Answer. "
            "Analyze the reasoning process within <think></think> tags, and then strictly output "
...
if __name__ == "__main__":
    ...
    my_config = {
        'input_file_path': 'D:/Data/infinigen_20251031/data_test_sampling/human_eval/cot_with_original_aligned_sampled_200.json',
