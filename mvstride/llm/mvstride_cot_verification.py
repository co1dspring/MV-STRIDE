...
class APIDataProcesser:
    def __init__(self, config):
        ...
        self.SYSTEM_PROMPT = """
        **Role:**
        You are a pragmatic, expert Verifier for 3D Multimodal Spatial Reasoning. Your task is to evaluate whether a candidate's Chain of Thought (CoT) is logically sound, physically plausible, and factually reasonable based on the provided Ground Truth (GT) observations and the final GT Answer.

        **Understanding the Input Structure:**
        1. **### 1. CORE TASK**: Contains the final spatial reasoning question and its standard `GT Answer`.
        2. **### 2. GROUND TRUTH OBSERVATIONS**: Sequential, verified factual QAs. These are the absolute physical and geometric benchmarks of the scene.
        3. **### 3. CANDIDATE COT TO BE VERIFIED**: The candidate reasoning path and predicted answer under audit.

        **Pragmatic & Tolerant Audit Rules (Do NOT be over-strict):**
        You are conducting a semantic and logical audit, not a literal text-matching. Please be reasonable and tolerant of minor, non-critical discrepancies:

        1. **Factual consistency**: Do the main spatial elements and relative camera movements align with Section 2?
           * **Tolerance**: Do NOT penalize for minor bounding box coordinate offsets (e.g., within +/- 100 pixels). Do NOT penalize for minor arithmetic rounding discrepancies in angles or distances (e.g., if GT says "13.81 degrees" and candidate estimates "13 degrees" or calculates "13.03 degrees", this is ACCEPTABLE). Do NOT penalize for natural language synonyms (e.g., "desk" vs "table", "whiteboard" vs "board").
        2. **Reasoning faithfulness**: Does the candidate logically build upon the Level 1/2 facts?
           * **Tolerance**: Do NOT penalize the candidate for mentioning real, visually verifiable details present in the images but omitted in the written GT (e.g., describing a "chessboard" on a table, or noting "windows on the left"), as long as these details are physically true and do not contradict the GT.
        3. **Final-answer consistency**: Does the candidate's final choice match the core `GT Answer` option?
        4. **Hallucination**: Only flag this if the candidate invents completely non-existent objects, assumes an entirely wrong camera viewpoint, or describes geometrically impossible/contradictory spatial relationships.

        **Verdict Selection:**
        - If the candidate's reasoning is logically sound, factually reasonable under the tolerant rules, and leads to the correct final option, output: `Correct`
        - Otherwise, identify the **single most critical and primary error** from:
          * `Factual inconsistency` (violates Criterion 1)
          * `Reasoning unfaithfulness` (violates Criterion 2)
          * `Final-answer inconsistency` (violates Criterion 3)
          * `Hallucination` (violates Criterion 4)

        **Output Protocol (STRICT):**
        You must place your internal analysis inside `<think></think>` tags. You MUST output your final verdict (exactly one of the 5 labels) inside `<answer></answer>` tags.

        Example Output Format:
        <think>
        [Your internal verification and reasoning here]
        </think>
        <answer>Correct</answer>

        *Do not include any other text, markdown, or explanation outside of these tags. The content inside <answer> must be ONLY the label.*
        """
    ...
    def worker_task(self, idx, item):
        ...
        try:
            ...
            if results is not None:
                ...
                # Attach the verification result to the original record.
                output_piece = item
                output_piece['verification_result'] = output
    ...
    def _format_user_prompt(self, item):
        ...
        the core question, the GT multi-turn factual dialogue, and the candidate
        CoT & answer to be verified.
        """
        # 1. Get and clean the core question.
        raw_final_question = item['messages'][0]['content']
        clean_final_question = raw_final_question

        # 2. Extract the candidate CoT and predicted answer.
        candidate_assistant_content = item['messages'][1]['content']

        # Separate the think and answer blocks with regex (robust to irregular formatting).
        candidate_think = "No think block found."
        candidate_answer = "No answer block found."

        think_match = re.search(r" thinking(.*?) response", candidate_assistant_content, re.DOTALL | re.IGNORECASE)
        if think_match:
            candidate_think = think_match.group(1).strip()

        ans_match = re.search(r"<answer>(.*?)</answer>", candidate_assistant_content, re.DOTALL | re.IGNORECASE)
        if ans_match:
            candidate_answer = ans_match.group(1).strip()
        else:
            # Fall back to all text after the think block when no answer tags are present.
            candidate_answer = re.sub(r" thinking.*? response", "", candidate_assistant_content, flags=re.DOTALL | re.IGNORECASE).strip()

        # 3. Build the prompt text.
        user_prompt = "Please verify the candidate reasoning process for the following task.\n\n"

        # Module A: core task.
        user_prompt += "### 1. CORE TASK\n"
        user_prompt += f"Question:\n{clean_final_question}\n\n"
        user_prompt += f"GT Answer:\n{item['original_ground_truth_answer']}\n\n"

        # Module B: multi-turn GT facts (primary supporting context).
        user_prompt += "### 2. GROUND TRUTH OBSERVATIONS (Multi-turn Context)\n"
        user_prompt += "These are verified correct physical observations and intermediate calculations for this scene:\n"
        user_prompt += "--------------------------------------------------\n"

        intermediate_qa = item.get("original_intermediate_qa", [])
        for idx, qa in enumerate(intermediate_qa, 1):
            q_clean = qa["question"]
            # Strip the leading "Answer XX:" prefix.
            a_clean = re.sub(r"^Answer\s*\d+:\s*", "", qa["answer"], flags=re.IGNORECASE).strip()

            user_prompt += f"Q{idx}: {q_clean}\n"
            user_prompt += f"A{idx}: {a_clean}\n\n"
        user_prompt += "--------------------------------------------------\n\n"

        # Module C: the candidate reasoning to be verified.
        user_prompt += "### 3. CANDIDATE COT TO BE VERIFIED\n"
        user_prompt += "Here is the reasoning path generated by the candidate model. Please inspect it carefully:\n"
        user_prompt += f"--- Candidate Think Process ---\n{candidate_think}\n\n"
        user_prompt += f"--- Candidate Predicted Answer ---\n{candidate_answer}\n"
        user_prompt += "--------------------------------------------------\n\n"

        user_prompt += (
            "Please compare the Candidate CoT with the GT Observations and the GT Answer. "
            "Analyze the reasoning process within  thinking response tags, and then strictly output "
            "your final single-label verdict inside <answer></answer> tags."
        )
