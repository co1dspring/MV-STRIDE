...
class Level2Mixin:
    def _object_correspondence(self, objects, cam1_data, cam2_data, obj_ids, cat, camera_name):
        # Determine the object in one image, then find its correspondence in the other image
        qa_type = "Object_correspondence"
        template = self.parent.templates[qa_type]
        H, W = get_image_size(cam1_data)

        if camera_name == "1,2":
            # locate the object in cam1
    ...
    def _cam_rot_yaw(self, objects, cam1_data, cam2_data, obj_ids, cat, camera_name):
        ...
        if camera_name == "1,2":
            # ask about the yaw rotation angle from cam1 to cam2
            R_rel = R2 @ R1.T
            question = template["question"].format(fig_num_1="1", fig_num_2="2")
        elif camera_name == "2,1":
            # ask about the yaw rotation angle from cam2 to cam1
            R_rel = R1 @ R2.T
    ...
    def _cam_rot_pitch(self, objects, cam1_data, cam2_data, obj_ids, cat, camera_name):
        ...
        if camera_name == "1,2":
            # ask about the pitch rotation angle from cam1 to cam2
            R_rel = R2 @ R1.T
            question = template["question"].format(fig_num_1="1", fig_num_2="2")
        elif camera_name == "2,1":
            # ask about the pitch rotation angle from cam2 to cam1
            R_rel = R1 @ R2.T
            question = template["question"].format(fig_num_1="2", fig_num_2="1")
    ...
    def _cam_trans_forward(self, objects, cam1_data, cam2_data, obj_ids, cat, camera_name):
        ...
        if camera_name == "1,2":
            # ask about the forward/backward movement from cam1 to cam2
            cam_for = - cam1_for
            ...
            # ask about the forward/backward movement from cam2 to cam1
            cam_for = - cam2_for
            # compute the displacement vector
        ...
        # project the displacement vector onto the local axes
    ...
    def _cam_trans_right(self, objects, cam1_data, cam2_data, obj_ids, cat, camera_name):
        ...
        # build cam1's local coordinate frame
