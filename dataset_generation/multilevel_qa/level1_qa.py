...
class Level1Mixin:
    ...
    def _cam_obj_yaw(self, objects, cam_data, obj_ids, cat, camera_name):
        ...
        # 2. Build the camera local coordinate frame axes
        # assume the world Z axis [0,0,1] is Up
        ...
        # 3. Project the displacement vector onto the local axes
        # D_f: how far in front of the camera the object is
        # D_r: how far to the right of the camera (positive right, negative left)
        D_f = np.dot(T_vec, cam_for)
        D_r = np.dot(T_vec, R_vec)

        # 4. Compute the horizontal angle (Yaw)
        # we only care about horizontal offset, so ignore the vertical difference
        # use arctan2(y, x), i.e. arctan2(right component, forward component)
        ...
        if abs_angle < 1.0:  # within 1 degree is considered center
            side = "center"
    ...
    def _3D_location_cam_obj(self, objects, cam_data, obj_ids, cat, camera_name):
        ...
        world_pos = np.append(ref_obj_loc, 1.0)  # convert to homogeneous coordinates [x, y, z, 1]
        cam_pos_homo = np.dot(P_w2c, world_pos)  # matrix multiplication
